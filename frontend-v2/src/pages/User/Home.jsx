...
const UserHome = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="relative glass p-8 rounded-2xl overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-accent-cyan text-sm font-medium">{greeting}</span>
          </div>
          <h1 className="text-3xl font-bold text-white mb-2">
            Welcome back, {user?.Name || 'there'} 👋
          </h1>
          <p className="text-white/50">Your complete AI-powered healthcare ecosystem.</p>
          <button onClick={() => navigate('/user/setup-profile')}
            className="mt-5 btn-primary inline-flex items-center gap-2 text-sm py-2">
      ...
      {/* ── Services ── */}
      <div>
        <h2 className="text-xl font-semibold text-white mb-5">Core Health Services</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {SERVICES.map(({ title, icon: Icon, desc, path, color }) => (
            <div key={title}
              onClick={() => path && navigate(path)}
              className={`glass p-6 rounded-2xl group ${path ? 'cursor-pointer hover:-translate-y-1 hover:border-white/20' : 'cursor-default'} transition-all duration-300`}
            >
              ...
              {path && (
                <div className="flex items-center gap-1.5 mt-4 text-primary-400 text-sm font-medium group-hover:gap-2.5 transition-all">
      ...
      <div className="glass p-8 rounded-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-500 to-accent-purple flex items-center justify-center flex-shrink-0 shadow-glow-primary">
            <Brain size={22} className="text-white" />
          </div>
          <div className="flex-1">
            <h3 className="font-bold text-white text-lg mb-1">Smart Health Insight</h3>
            <p className="text-white/50 text-sm">Based on your profile, maintaining regular checkups and updating your health records improves prediction accuracy by up to 40%.</p>
          </div>
          <button onClick={() => navigate('/user/setup-profile')} className="btn-primary flex-shrink-0 text-sm py-2.5">
            Update Profile
          </button>
        </div>
      </div>
