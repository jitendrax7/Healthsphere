...
import { Search, Sun, Moon, Globe, Bell, Leaf, Menu } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { useTranslation } from 'react-i18next';
import { userSettingsApi } from '../../api/axios';

const Topbar = ({ onMenuToggle }) => {
  const { theme, setTheme, setLanguage, notificationCount, user } = useApp();
  ...
  return (
    <div className="topbar-root h-16 border-b border-white/10 flex items-center justify-between px-4 md:px-8 bg-dark-900/80 backdrop-blur-xl z-30 sticky top-0 transition-colors duration-300">
      <div className="flex items-center flex-1 min-w-0">
        {user.role == 'user' && <button
          onClick={onMenuToggle}
          className="md:hidden p-2 -ml-2 mr-2 text-white/70 hover:text-white rounded-lg hover:bg-white/10 transition-colors"
        >
          <Menu size={24} />
        </button>}
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-3 ml-2">
        ...
        <div
          className={`flex items-center transition-all duration-300 overflow-hidden ${searchOpen ? 'w-40 sm:w-48 md:w-64 glass px-3 py-2' : 'w-10 h-10 justify-center hover:bg-white/10 rounded-full cursor-pointer'}`}
        ...
        <div className="relative group cursor-pointer flex items-center justify-center w-10 h-10 rounded-full hover:bg-white/10 text-white/60 hover:text-white transition-colors">
          ...
          <div className={`absolute top-[80%] right-0 mt-3 w-36 backdrop-blur-md border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 overflow-hidden shadow-xl rounded-xl py-1 z-50 ${
            ...
            {[
              ...
              >
                {lang.label}
                {currentLanguage === lang.code && <div className="w-1.5 h-1.5 rounded-full bg-primary-500 shadow-glow-primary" />}
              </div>
            ))}
          </div>
        </div>
