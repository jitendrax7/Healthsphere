...
import {
  ...
  LogOut, ChevronLeft, ChevronRight, User, Settings, X
} from 'lucide-react';
...
const Sidebar = ({ mobileOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed left-0 top-0 h-screen bg-dark-800/95 backdrop-blur-xl border-r border-white/10
      ...
      `}>
        ...
        <div className={`flex items-center gap-3 p-5 border-b border-white/10 ${collapsed ? 'justify-center' : ''}`}>
          ...
          {!collapsed && (
            <button
              onClick={onClose}
              className="ml-auto md:hidden text-white/40 hover:text-white p-1 rounded-lg hover:bg-white/10 transition-colors"
            >
              <X size={18} />
            </button>
          )}
        ...
        <button
          ...
          className="absolute -right-3 top-16 w-6 h-6 bg-dark-600 border border-white/20 rounded-full hidden md:flex items-center justify-center text-white/60 hover:text-white hover:bg-primary-600 transition-all duration-200 z-50"
        >
        ...
        <div className="p-3 pb-24 md:pb-3 border-t border-white/10 space-y-3">
          {!collapsed && (
            <button
          ...
          {user && (
            <>
                ...
                {profileMenuOpen && (
                  <>
                    ...
                    <div className={`absolute bottom-[calc(100%+10px)] glass-dark border border-white/10 rounded-xl p-2 shadow-2xl animate-fade-in z-50 ${collapsed ? 'left-14 w-48' : 'left-0 w-full'}`}>
                      <button
                        onClick={() => { setProfileMenuOpen(false); handleNavClick(); navigate('/user/profile'); }}
                      ...
                      <button
                        onClick={() => { setProfileMenuOpen(false); handleNavClick(); navigate('/user/settings?tab=general'); }}
                        className="w-full flex items-center gap-3 p-2.5 rounded-lg hover:bg-white/10 text-white text-sm font-medium transition-colors text-left"
                      >
              ...
              {/* Mobile Logout Button (Settings/Profile moved to Bottom Nav) */}
              <button
                onClick={handleLogout}
                className="md:hidden w-full p-3 rounded-xl flex items-center justify-center gap-2 border border-red-500/20 bg-red-500/10 text-red-500 font-bold transition-all hover:bg-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.1)] mt-2"
              >
                <LogOut size={16} className="opacity-90" /> {t('Log Out')}
              </button>
            </>
          )}
        </div>
